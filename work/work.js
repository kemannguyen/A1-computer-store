...
function transferMoneyToBank(debt, currBalance, payBalance, payBtnElement) {
  //if the user haven't earned anything yet
  if (payBalance === 0) {
    alert("earn some money first please");
    return;
  }
  if (debt > 0) {
    ...
    if (debt >= payBalance / 10) {
      ...
      //see how much money there is left from the paybalance if the 10% of the transfer is more than the debt
      debt -= payBalance;
      debt *= -1;
      currBalance += debt;
      debt = 0;
